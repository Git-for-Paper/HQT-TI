...
public class Region implements IShape, Serializable
{
    ...
    public double getMaximumDistance(final Point p)
    {
        double max = Double.NEGATIVE_INFINITY;

        double d = Math.pow(p.m_pCoords[0]-m_pLow[0], 2)+Math.pow(p.m_pCoords[1]-m_pLow[1], 2);
        max = Math.max(max, d);

        d = Math.pow(p.m_pCoords[0]-m_pHigh[0], 2)+Math.pow(p.m_pCoords[1]-m_pLow[1], 2);
        max = Math.max(max, d);

        d = Math.pow(p.m_pCoords[0]-m_pHigh[0], 2)+Math.pow(p.m_pCoords[1]-m_pHigh[1], 2);
        max = Math.max(max, d);

        d = Math.pow(p.m_pCoords[0]-m_pLow[0], 2)+Math.pow(p.m_pCoords[1]-m_pHigh[1], 2);
        max = Math.max(max, d);
    ...
    public Region getIntersectRegion(final Region r)
    {
        if (m_pLow.length != r.m_pLow.length) throw new IllegalArgumentException("Shape has the wrong number of dimensions.");
        if(intersects(r)==false) throw new IllegalArgumentException("Don't intersect");
        double[] mn = new double[m_pLow.length];
        double[] mx = new double[m_pLow.length];
        for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)
        {
            mn[cIndex] = Math.max(m_pLow[cIndex],r.m_pLow[cIndex]);
            mx[cIndex] = Math.min(m_pHigh[cIndex],r.m_pHigh[cIndex]);
        }
