...
{
    public Index(DIRTree pTree, int id, int level)
    {
        super(pTree, id, level, pTree.m_indexCapacity);
    }

    protected Node chooseSubtree(Region mbr, int level, Stack pathBuffer, Vector inv)
    {
        ...
        switch (m_pTree.m_treeVariant)
        {
            ...
            case SpatialIndex.RtreeVariantQuadratic:
                child = findLeastEnlargement(mbr, inv);
                break;
        ...
        Node ret = n.chooseSubtree(mbr, level, pathBuffer, inv);
    ...
    protected Node[] split(byte[] pData, Region mbr, int id) throws Exception
    {
    ...
    protected int findLeastEnlargement(Region r, Vector inv)
    {
        ...
        for (int cChild = 0; cChild < m_children; cChild++)
        {
            ...
            ///////////////////////////////////////////////////////////
            HashSet centerv = null;
            try{
                centerv = (HashSet)DIRTree.docbtree.find(m_pIdentifier[cChild]);
            }
            catch(Exception e){
                e.printStackTrace();
            }

            double score = enl*DIRTree.alpha_dist + (1-DIRTree.alpha_dist)*HammingDist(inv, centerv)/centerv.size();
            if(score < minscore){
                minscore = score;
                best = cChild;
            }
    ...
    protected void adjustTree(Node n, Stack pathBuffer)
    {
        ...
        HashSet inv;
        try{
            v = (HashSet)DIRTree.docbtree.find(this.m_identifier);
            inv = (HashSet)DIRTree.docbtree.find(n.m_identifier);

            if(v == null)
                DIRTree.docbtree.insert(this.m_identifier, inv, true);
            else{
                v.addAll(inv);
                DIRTree.docbtree.insert(this.m_identifier, v, true);
            }
        }
        catch(Exception e){
            e.printStackTrace();
    ...
    protected void adjustTree(Node n1, Node n2, Stack pathBuffer, boolean[] overflowTable) throws Exception
    {
        m_pTree.m_stats.m_adjustments++;
        ...
        try{
            v = (HashSet)DIRTree.docbtree.find(this.m_identifier);
            inv = (HashSet)DIRTree.docbtree.find(n1.m_identifier);
